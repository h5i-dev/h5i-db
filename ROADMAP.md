...
| 2.9 | **Session/cache tension** — fresh data needs a new `H5iSession`, which drops the footer-metadata cache (~40% of warm latency) and re-resolves tables serially. Add shared `Arc<RuntimeEnv>` option, `refresh()`, concurrent resolves. | `query/src/session.rs:47-137` |
...
| # | Item | Where / fix |
|---|------|-------------|
| 3.1 | **Nothing is published** — `pip install h5i-db` and the crates.io path in README don't work; release.yml only attaches to GitHub Releases. Publish (maturin publish + cargo publish) or fix the docs to say "from source". | `.github/workflows/release.yml`, `README.md:19` |
...
| 3.3 | **No version GC/retention** — every historical version pins its segments forever; unbounded storage, compliance deletion impossible, vacuum cost O(V). Add `expire_versions(retain_last_n \| retain_since_ts)` respecting snapshot pins. | `core/src/database.rs:1345-1351` |
| 3.4 | **Vacuum edges**: orphaned table dirs not in the catalog are unreachable forever; `grace=0` can destroy an in-flight commit (enforce minimum ≥ lock timeout); `plan_protected_paths` fails *open* on storage errors. | `core/src/database.rs:1330-1341,1361-1370`, `core/src/plan.rs:503-509` |
...
3. **Incremental queries between versions** — manifests already store
   ...
   set-difference away. A genuine differentiator nobody else has cheaply; nothing
   exposes it. Makes incremental OHLCV maintenance nearly free.
...
- **Benchmarks**: re-run on non-WSL bare-metal x86_64 with symmetric methodology
  (h5i-db is single-run, Polars best-of-3 — indefensible either way; use median of
  N≥5 both sides), add the promised DuckDB and ArcticDB baselines, a Polars
  `set_sorted` variant to preempt the obvious objection, and a scaling curve
  (segments → thousands, versions → 10⁴).
