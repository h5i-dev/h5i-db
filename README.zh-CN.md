...
- **原生支持主流市场：** Kalshi、Polymarket 和 Hyperliquid 的报文都会解码成
  同一套规范表，费用也按各场所真实的费率曲线和资金费计算。
...
各家的载荷都解码进同一组规范表——`instruments`、`book_deltas`、`trades`、
`bars`、`funding`、`references`、`resolutions`——所以内核不需要认识任何一家的
数据形状。凡是能写这几张表的都是加载器，用什么语言都行。下面这三家自带了针对
真实载荷录制样本的解析器与测试。

| | Kalshi | Polymarket | Hyperliquid |
|---|---|---|---|
| 合约元数据 | REST 市场信息 | 一个市场 N 个结果 | 永续与现货两套universe |
| L2 盘口 | REST + WS 快照，带序号校验的增量 | `book` 快照，`price_change` 增量 | REST + WS |
| 成交 | ✓ 实时与历史共用一个解析器 | ✓ | ✓ |
| K 线 | ✓ candlesticks | ✗ | ✓ candles |
| 资金费率 | 不适用 | 不适用 | ✓ |
| 标记价与预言机价 | 不适用 | 不适用 | ✓ 资产上下文 |
| 保证金与杠杆 | 不适用 | 不适用 | ✓ 每币种上限、仅逐仓标记 |
| 结算 | ✓ 可观测之后 | ✓ 来自市场结果 | 不适用 |
| 完整份额的 mint / redeem | ✗ | ✓ neg-risk 市场 | 不适用 |
| 费用模型 | 二次式，含交易所取整 | 比例 + 做市返佣 | 14 天滚动成交量分档 |
| 历史归档 | ✗¹ | pmxt、telonex、Kaggle 三种布局 | 每小时全量feed + 资产上下文 |

¹ Kalshi 不公开历史盘口增量，所以要做到队列位置准确的回测，得提前把带鉴权的
WebSocket 流录下来。序号缺失时会立起一个 gap，而不是在缺口上插值。

“不适用”指该场所没有这个概念：永续不会结算，全额抵押的预测市场也没有杠杆和资金费率。
