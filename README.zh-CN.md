...
并非直接计时。
⁷ `--features wide`，让定点类型变成 `i128`。默认关闭，详见
[精度与范围](https://db.h5i.dev/manual/backtest/)。内核这一项是上一行乘以
**1.43 倍**：在同一台机器上交替测了 12 对，12 次都是 `wide` 更慢，中位数从
68.2 ms 到 97.5 ms，两组的区间没有重叠。用倍率而不直接引用实测值，是因为这台
机器跑 i64 内核是 68.2 ms 而非 65.7 ms，这个差属于机器。回调那一行的边界开销
照旧沿用：跨进 Python 与整数宽度无关。
⁸ 是没有变化，不是没有测。同样这 12 对里，`wide` 更慢的有 6 次、更快的也有 6
次，中位数相差在 2% 以内。这一行由每次提交要付的 fsync 主导，与运算宽度无关。
