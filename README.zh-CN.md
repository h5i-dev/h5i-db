...
- **跑在同一套存储上的事件驱动回测器：** 重放内核每秒处理 305 万个事件，在同一
  份盘口最优价工作负载上是 NautilusTrader 的 11.7 倍、LEAN 的 31 倍。一次运行在
  fork 内部执行，并把订单、成交、持仓和净值曲线写回成可以直接查询的普通表。
- **会一并交代自身可信度的统计：** 与 `alphalens` 对齐的因子评价、与 `empyrical`
  对齐的绩效统计，外加 deflated Sharpe、回测过拟合概率（PBO），以及 purged 与
  combinatorial 交叉验证。
