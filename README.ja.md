...
- **ターミナルの中のノートブック:** エージェントも人も、ターミナルを出ずに
  探索的な分析ができる。`h5i-db nb view` は普通の `.ipynb` を本物のJupyter
  カーネルで実行し、matplotlibのプロットをその場に描く。
...
- **ターミナルの中のノートブックが効くのは、クオンツがJupyterを使うのと同じ理由。**
セルを部分的に実行できる、カーネルがステップ間で状態を保つ、結果がそれを生んだ
コードの隣に残る。この3つをエージェントはコマンドラインから、`nb exec` と
`nb run --cells 3-7` で手に入れる。
