...
⁷ `--features wide` で固定小数点の型が `i128` になる。既定では無効。詳しくは
[精度と範囲](https://db.h5i.dev/manual/backtest/)。カーネルの数値は、一つ上の行に
**1.43倍**を掛けたもの。この倍率は同じマシンで交互に12回計測した中央値で、12回とも
`wide` のほうが遅く（中央値 68.2 → 97.5 ms）、両者の範囲は重ならなかった。直接の実測値
ではなく倍率で出した理由は、このマシンの i64 カーネルが 68.2 ms で、公表値の
65.7 ms とずれるからで、その差はマシンに属する。コールバック行の境界費用はそのまま使っている。
Python へ渡る費用は整数幅に依存しない。
⁸ この行は `wide` でも変わらなかった。同じ12回のうち遅かったのは6回、
速かったのも6回で、中央値の差は2%以内。この行はコミットが払う fsync が支配的で、
そこは演算の幅と関係がない。
