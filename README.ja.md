...
[精度と範囲](https://db.h5i.dev/manual/backtest/)。カーネルの数値は一つ上の行に
**1.43倍**を掛けたもの。同じマシンで交互に12回計測し、12回とも `wide` が遅く
（中央値 68.2 → 97.5 ms）、両者の範囲は重ならなかった。倍率にしたのは、このマシンの
i64 カーネルが 68.2 ms で、公表値の 65.7 ms とずれるから。Python 境界の費用はそのまま。
⁸ 計測した上で、変わらなかった。12回のうち `wide` が遅かったのは6回、速かったのも
6回で、中央値の差は2%以内。この行はコミットの fsync が支配的で、演算の幅とは関係がない。
