...
- **同じストレージの上で動くイベント駆動バックテスタ:** リプレイカーネルは毎秒305万
  イベントを処理し、同一ワークロードでNautilusTraderの11.7倍、LEANの31倍。実行は
  forkの中で走り、注文・約定・ポジション・エクイティカーブをそのままクエリできる
  テーブルとして書き戻す。
- **統計が自分の信頼度も一緒に返す:** `alphalens` と一致するファクター評価、
  `empyrical` と一致するパフォーマンス統計に加えて、deflated Sharpe、
  バックテスト過剰適合確率（PBO）、purged/combinatorialのクロスバリデーション。
