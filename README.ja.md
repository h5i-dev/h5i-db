...
- **主要な市場をそのまま扱える:** Kalshi、Polymarket、Hyperliquidのペイロードは
...
ペイロードは正規化された一組のテーブル（`instruments`、`book_deltas`、
`trades`、`bars`、`funding`、`references`、`resolutions`）へデコードされる。
だからカーネルは取引所ごとの形を知らない。このテーブルを書けるものは何であれ
ローダで、言語も問わない。以下の3つは記録済みペイロードに対するテスト付きの
パーサを同梱している。

| | Kalshi | Polymarket | Hyperliquid |
|---|---|---|---|
| 銘柄メタデータ | RESTのマーケット情報 | 1マーケットにN個のアウトカム | 無期限先物とスポットの両ユニバース |
| L2板 | REST + WSスナップショット、シーケンス検査付きの差分 | `book` スナップショットと `price_change` 差分 | REST + WS |
| 約定 | ✓ ライブと履歴を同一パーサで | ✓ | ✓ |
| ローソク足 | ✓ candlesticks | ✗ | ✓ candles |
| ファンディング | 該当なし | 該当なし | ✓ |
| マーク価格・オラクル価格 | 該当なし | 該当なし | ✓ 資産コンテキスト |
| 証拠金とレバレッジ | 該当なし | 該当なし | ✓ 銘柄ごとの上限、isolated限定フラグ |
| 決済 | ✓ 観測可能になった時点で | ✓ マーケットのresolutionから | 該当なし |
| コンプリートセットのmint/redeem | ✗ | ✓ neg-riskマーケット | 該当なし |
| 手数料モデル | 二次式、取引所の丸め込み込み | 比例＋メイカーリベート | 14日ローリング出来高のティア |
| 履歴アーカイブ | ✗¹ | pmxt、telonex、Kaggleの各レイアウト | 毎時フィード＋資産コンテキスト |

¹ Kalshiは板の履歴差分を公開していないので、キュー位置まで正確なバックテストには
認証付きWebSocketを事前に取り込んでおく必要がある。シーケンス番号が欠けたときは、
間を補間せずギャップとして立てる。

「該当なし」はその取引所にその概念がないという意味。無期限先物は決済しないし、
全額担保の予測市場にレバレッジもファンディングもない。
