...
Aggregates and scalars are methods: `.sum()`, `.mean()`, `.min()`, `.max()`,
`.count()`, `.n_unique()`, `.std()`, `.var()`, `.median()`, `.quantile(q)`,
`.first(order_by=)`, `.last(order_by=)`, `.abs()`, `.log()`, `.sqrt()`,
`.round(n)`, `.floor()`, `.ceil()`, `.coalesce(...)`. Plus `count_star()`,
`vwap(price, size)`, `wavg(weight, value)` and `time_bucket(interval, ts)` as
functions.
